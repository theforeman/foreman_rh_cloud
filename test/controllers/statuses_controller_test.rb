...
class StatusesControllerTest < ActionController::TestCase
  tests ForemanYupana::StatusesController
  ...
  test 'Returns statuses for each process type' do
    ...
    generate_label = ForemanYupana::Async::GenerateReportJob.output_label('test')
    generate_output = ForemanYupana::Async::ProgressOutput.register(generate_label)
    generate_output.status = 'generate_status_test'
    upload_label = ForemanYupana::Async::UploadReportJob.output_label('test')
    upload_output = ForemanYupana::Async::ProgressOutput.register(upload_label)
