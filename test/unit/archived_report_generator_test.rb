...
class ArchivedReportGeneratorTest < ActiveSupport::TestCase
  ...
  test 'generates a report for a single host' do
    ...
    InventoryUpload::Generators::Queries.expects(:for_report).with(portal_user).returns(batches)
    Dir.mktmpdir do |tmpdir|
      target = File.join(tmpdir, 'test.tar.gz')
      generator = InventoryUpload::Generators::ArchivedReport.new(target, Logger.new(STDOUT))
