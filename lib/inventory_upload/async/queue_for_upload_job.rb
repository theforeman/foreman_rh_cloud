  ...
  module Async
    class QueueForUploadJob
      ...
      def perform(report_file, portal_user)
        ...
        enqueued_file_name = File.join(uploads_folder, InventoryUpload.facts_archive_name)
      ...
      def uploads_folder
        @uploads_folder ||= InventoryUpload.uploads_folder(@portal_user)
      end

      def script_file
        @script_file ||= File.join(uploads_folder, InventoryUpload.upload_script_file)
      end
      ...
      def ensure_output_script
        ...
        script_source = File.join(InventoryUpload::Engine.root, 'app/views/scripts/uploader.sh.erb')

        template_src = Foreman::Renderer::Source::String.new(content: File.read(script_source))
        scope = Foreman::Renderer::Scope::Base.new(
          source: template_src,
          variables: {
            upload_url: InventoryUpload.upload_url,
